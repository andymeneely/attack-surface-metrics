...
class CallGraphTestCase(unittest.TestCase):
    ...
    def test_get_entry_surface_metrics(self):
        ...
        expected_values.append(
            {
                'node': 'recursive_a',
                'points': [greet_b], 'proximity': 2, 'surface_coupling': 1
            }
        )
        expected_values.append(
            {
                'node': 'greet',
                'points': [greet_b], 'proximity': 1, 'surface_coupling': 1
            }
        )
        expected_values.append(
            {
        ...
        expected_values.append(
            {
                'node': 'greet_b',
                'points': None, 'proximity': 0, 'surface_coupling': None
            }
        )
        expected_values.append(
            {
        ...
        for expected_value in expected_values:
            ...
            call = next(
                ...
                if node.function_name == expected_value['node']
            )
            ...
            if expected_value['points']:
                self.assertTrue(
                    all([
                        p in actual_value['points']
                        for p in expected_value['points']
                    ]),
                    msg=msg
                )
            else:
            ...
            self.assertEqual(
                expected_value['proximity'],
                actual_value['proximity'],
                msg=msg
            )
            self.assertEqual(
                expected_value['surface_coupling'],
                actual_value['surface_coupling'],
                msg=msg
            )

    def test_get_exit_surface_metrics(self):
        # Arrange
        greet = Call.from_cflow(
            "greet() <void greet (int greeting_code) at ./src/greetings.c:14>:"
        )
        recursive_b = Call.from_cflow(
            "recursive_b() <void recursive_b (int i) at ./src/greetings.c:32> ("
            "R): [see 23]"
        )
        recursive_a = Call.from_cflow("recursive_a() <void recursive_a (int i) "
            "at ./src/greetings.c:26> (R): [see 22]"
        )
        main = Call.from_cflow("main() <int main (void) at ./src/helloworld.c:5"
            "8> [see 3]"
        )
        GreeterSayHiTo = Call.from_cflow(
            "GreeterSayHiTo() <void GreeterSayHiTo (int value) at ./src/hellowo"
            "rld.c:53>:"
        )
        GreeterSayHi = Call.from_cflow("GreeterSayHi() <void GreeterSayHi () at"
            " ./src/helloworld.c:48>:"
        )
        ...
        expected_values.append(
            {
                'node': 'greet_a',
                'points': [greet, recursive_a, recursive_b],
        ...
        expected_values.append(
            {
                'node': 'new_Greeter',
                'points': [GreeterSayHi, GreeterSayHiTo],
                'proximity': 1, 'surface_coupling': 2
            }
        )
        expected_values.append(
            {
